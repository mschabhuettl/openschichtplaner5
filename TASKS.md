...
- 2026-05-27 18:20 · PR #154 · TEST/SECURITY: Error-Pfade des ORM-Mirror-Routers abgedeckt (orm_mirror.py 76%→95%) — parametrisierter Test verifiziert, dass **jeder** Endpoint interne Fehler sanitisiert (kein Leak im 500-Response); 19 Tests; Suite grün (2528)
...
- 2026-05-27 21:05 · PR #159 · FIX/SECURITY: fehlende `limit`-Grenzen auf List-Endpoints — `GET /api/changelog` hatte keine Obergrenze (Lib-`get_changelog` macht rohes `stmt.limit(limit)` ohne Clamp → `limit=999999` materialisiert das ganze Audit-Log; DoS-Vektor) und kein `ge=1` (SQLite `LIMIT -1` = unbegrenzt). Jetzt `ge=1, le=5000` (analog `admin.py` Audit-Events); `/api/notifications` + `/api/notifications/all` um `ge=1` ergänzt. 9 Regressionstests (`TestListLimitBounds`); rein restriktiv, keine Verhaltensänderung für gültige Requests. Suite grün
