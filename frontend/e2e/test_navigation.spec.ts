...
test.describe('Navigation Tests (als admin)', () => {
  ...
  test('Alle Hauptseiten laden ohne Fehler', async ({ page }) => {
    ...
    page.on('console', msg => {
      if (msg.type() === 'error') {
        const text = msg.text();
        // Filter expected network/API errors — only catch real JS errors
        const isNetworkError = text.includes('401') || text.includes('403') ||
          text.includes('404') || text.includes('500') ||
