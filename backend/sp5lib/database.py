...
class SP5Database:
    ...
    def generate_schedule_from_cycle(
        ...
        Gibt zurück: {'created': N, 'skipped': N, 'errors': [...]}
        """
            ...
            for day in range(1, num_days + 1):
                ...
                if key in existing_entries:
                    if not force:
                        skipped += 1
                        continue
                    # force=True: delete existing entry first
                    try:
    ...
    def get_schedule_conflicts(self, year: int, month: int, group_id: int = None) -> list:
        ...
        # Build: employee_id -> set of dates with shifts
        shift_dates: dict = {}
        for r in self._read('MASHI'):
            d = r.get('DATE', '')
            if d and d.startswith(prefix):
        ...
        # Build: employee_id -> list of absence date strings
        absence_dates: dict = {}
        for r in self._read('ABSEN'):
            d = r.get('DATE', '')
            if d and d.startswith(prefix):
